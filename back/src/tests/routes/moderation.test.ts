...
describe("Moderation Routes - Version-Aware Approval Tests", () => {
  ...
  describe("getPendingSubmissions with version info", () => {
    test("returns version field for projects", async () => {
      const project = await TestHelpers.createTestProject(testUser.id, testCity.id, 1, {
        status: "pending",
      });
  ...
  describe("setProjectApprovalStatusWithVersion", () => {
    ...
    test("handles project with mixed version states individually", async () => {
      const project1 = await TestHelpers.createTestProject(testUser.id, testCity.id, 1, {
        status: "pending",
        name: "Test Project 1",
      });
      const project2 = await TestHelpers.createTestProject(testUser.id, testCity.id, 1, {
        status: "pending",
        name: "Test Project 2",
      });
      ...
      const db = getTestDb();
      const [finalProject1] = await db
        .select()
        .from(projects)
        .where(eq(projects.id, project1.id))
        .limit(1);
      const [finalProject2] = await db
        .select()
        .from(projects)
      ...
      const submissionData = await moderationRouter
        .createCaller(TestHelpers.createAdminContext(testAdmin.id))
  ...
  describe("setOverlayApprovalStatusWithVersion", () => {
    test("approves overlay with correct version", async () => {
      const project = await TestHelpers.createTestProject(testUser.id, testCity.id);
      const overlay = await TestHelpers.createTestOverlay(project.id, testUser.id, 1, {
        status: "pending",
      });
  ...
  describe("Edge cases and error handling", () => {
    test("handles individual approval calls", async () => {
      ...
      const overlay = await TestHelpers.createTestOverlay(overlayProject.id, testUser.id, 1, {
        status: "pending",
      });
