...
async function getChangeRequestLocation(
  tx: Transaction,
  request: SubmissionBatchInput["changeRequests"][number],
  projectId: string,
  ...
  if (request.entityType === "project") {
    const entity = await tx
      ...
      .from(projects)
      .where(eq(projects.id, request.entityId))
      .limit(1);
    if (!entity[0]) {
      throw new TRPCError({ code: "NOT_FOUND", message: "Project not found" });
    }
    if (entity[0].status !== "approved") {
      throw new TRPCError({
        code: "BAD_REQUEST",
        message: "Change requests can only be submitted for approved projects",
      });
    }
    return { lat: entity[0].lat, lng: entity[0].lng };
  }
  ...
  const entity = await tx
    ...
    .from(overlays)
    .where(eq(overlays.id, request.entityId))
    .limit(1);
  if (!entity[0]) {
    throw new TRPCError({ code: "NOT_FOUND", message: "Overlay not found" });
  }
  if (entity[0].projectId !== projectId) {
...
async function loadDirectSubmissionState(
  tx: Transaction,
  input: SubmissionBatchInput,
  userId: string,
) {
  const directOverlayIds = [
    ...input.overlays.map((overlay) => overlay.id),
    ...(input.render ? [input.render.id] : []),
  ];
  const existingProjectRows = input.project?.id
    ? await tx
  ...
  const existingOverlayRows =
    directOverlayIds.length > 0
      ? await tx
          ...
          .from(overlays)
          .where(inArray(overlays.id, directOverlayIds))
      : [];
  ...
  const directExistingStatuses = [
    ...(input.project ? [existingProject?.status] : []),
    ...input.overlays.map((overlay) => existingOverlays.get(overlay.id)?.status),
    ...(input.render ? [existingOverlays.get(input.render.id)?.status] : []),
  ];
  const pendingAdditions = directExistingStatuses.filter((status) => status !== "pending").length;
  const totalAdditions = directExistingStatuses.filter(
    (status) => status !== "pending" && status !== "approved",
  ).length;
  const currentPending = (pendingProjects?.count ?? 0) + (pendingOverlays?.count ?? 0);
  const currentTotal = (totalProjects?.count ?? 0) + (totalOverlays?.count ?? 0);
  if (currentPending + pendingAdditions > MAX_PENDING_CONTRIBUTIONS) {
    throw new TRPCError({
      code: "BAD_REQUEST",
  ...
  if (currentTotal + totalAdditions > MAX_TOTAL_CONTRIBUTIONS) {
    throw new TRPCError({
      code: "BAD_REQUEST",
      message: `You have reached the maximum lifetime limit of ${MAX_TOTAL_CONTRIBUTIONS} contributions.`,
    });
  }
  return { existingProject, existingOverlays };
}

type Notification = Parameters<typeof notifyNewSubmission>[0];
type DirectSubmissionState = Awaited<ReturnType<typeof loadDirectSubmissionState>>;
...
async function writeProject(
  ...
  slug: string | null,
  user: SessionUser,
  existing: DirectSubmissionState["existingProject"],
...
async function writeOverlay(
  ...
  projectId: string,
  user: SessionUser,
  existingOverlays: DirectSubmissionState["existingOverlays"],
  notifications: Notification[],
...
async function writeChangeRequest(
  ...
  const { lat, lng } = await getChangeRequestLocation(tx, request, projectId);
...
export const submissionRouter = router({
  submit: loggedInProcedure.input(submissionBatchSchema).mutation(async ({ input, ctx }) => {
    try {
      ...
      const directProject = input.project;
      const countryCode = directProject
        ? await resolveCountryCode(directProject.lat, directProject.lng)
        : null;
      if (directProject && !countryCode) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "Could not determine a country for this location",
        });
      }
      const projectSlug = directProject?.id
        ? buildProjectSlug({ name: directProject.name, externalId: null, id: directProject.id })
        : null;

      const committed = await db.transaction(async (tx) => {
        const { existingProject, existingOverlays } = await loadDirectSubmissionState(
          tx,
          input,
          userId,
        );
        ...
        for (const request of input.changeRequests) {
          await writeChangeRequest(tx, request, input.projectId, ctx.user, notifications);
        }
      ...
      if (directProject?.id) await assignProjectBoundary(directProject.id);
